"""HTTP request and response logging for the Message service."""
import logging
import time
...
class RequestLoggingMiddleware:
    ...
    def __call__(self, request):
        ...
        message = "HTTP request completed method=%s path=%s status=%s duration_ms=%.2f"; args = (request.method, request.path, response.status_code, elapsed_ms)
